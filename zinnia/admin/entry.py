...
class EntryAdmin(admin.ModelAdmin):
    ...
    #         print((author.get_absolute_url(), getattr(author, author.USERNAME_FIELD))for author in entry.authors.all())
    #
    ...
    def formfield_for_manytomany(self, db_field, request, **kwargs):
        ...
        if db_field.name == 'authors':
            kwargs['queryset'] = Author.objects.filter(Q(entries__isnull=False)
            ).distinct()
